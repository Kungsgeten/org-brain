...
- =brain:= :: The default kind of link. Just let's you visit another =org-brain=
  ...
  also be created as a resource in the link target, linking back to the entry
  where the link was created. If =org-brain-backlink= is set to a string, that
  string will be added as a prefix to the title of the backlink. *Example:* You
  set =org-brain-backlink= to ="<-- "= and create a =brain:= link in =Rabbits= linking
  to =Carrots=. Now a resource with the description =<-- Rabbits= will be created in
  =Carrots=.
